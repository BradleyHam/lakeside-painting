    ...
    <div className="group h-full shadow-[0_20px_25px_-5px_rgba(0,0,0,0.08),0_10px_10px_-5px_rgba(0,0,0,0.08)]">
      <Link href={`/projects/${slug}`} className="w-full h-full flex flex-col">
        <div className='card-image h-[250px] relative overflow-hidden '>
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#3D4D5C]/80 to-[#3D4D5C]/10 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100">
          </div>
        ...
        <div className="card-content px-6 py-6 flex flex-col items-start flex-grow bg-white text-primary ">
         <div className="tags flex flex-wrap items-center mb-3 gap-3">
            {tags.map((tag, index) => (
              <div key={index} className="bg-primary/10 text-primary/70 rounded inline-block px-[10px] py-[6px] cursor-pointer font-semibold text-xs capitalize">
                {tag}
              </div>
            ))}
          </div>
